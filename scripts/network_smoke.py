...
and a failure against production read identically:
    ...
    Production     python scripts/network_smoke.py --base-url https://emojimart.2plot.dev
...
# The default UA names the BROWSER lane first (template 1.6.40, found by
# muischeduler on its item-12 port): at dash-improve-my-llms >= 2.8 a UA
# with no browser engine token is classified crawler-lane, so a bare
# internal token made every default-UA check read the prerendered crawler
# document — a manifest-link or og:image check goes red the moment a floor
# moves, in CD's verify job. The internal token stays IN the string, after
# the engine token: INTERNAL_UA_TOKEN is a substring match, so the far
# side's internal-traffic exclusion still holds. CRAWLER_UA is the other
# lane and is deliberately untouched.
BROWSER_UA = (
...
SITE_H1 = "# dash-emoji-mart — emoji picker for Dash"

# The container port. Matches the Dockerfile's EXPOSE and CMD.
DEFAULT_BASE_URL = "http://localhost:8050"

# A page that certainly exists here, for the per-page llms.txt checks. The
# donor uses its own "/backends"; a fork that forgets to change this gets three
# failures that look like broken content negotiation and are really a 404.
SAMPLE_PAGE = "/popover"
...
def satellite_checks(base: str) -> None:
    ...
    def robots_artifact_fingerprint():
        ...
        # 2.3.2 allowed OAI-SearchBot; 2.3.3 split the vendors per bucket,
        # allowing Claude-User / Claude-SearchBot. The training crawlers are
        # checked separately below: their shape is POSTURE, not artifact.
        ...
        # Posture, not artifact (template 1.6.37): with the training wall
        # retired the package emits no training stanza at all, so absent and
        # Allow are both the allow shape — assert the ABSENCE of Disallow, not
        # the presence of Allow, or the check raises on the shape it wanted.
        for agent in ("ClaudeBot", "GPTBot"):
            marker = f"User-agent: {agent}"
            walled = marker in lines and lines[lines.index(marker) + 1] == "Disallow: /"
            expect(not walled,
                   f"{agent} still Disallowed: the posture flip has not landed")
