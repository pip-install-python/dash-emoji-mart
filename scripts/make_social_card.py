...
# Palette. The template reads these off assets/favicon/site.webmanifest; this
# repo has no manifest, so the accent comes from the two places that DO carry
# this site's colour and must not disagree with the card:
#   templates/index.html   <meta name="theme-color" content="#fab005">
#   lib/constants.py       PRIMARY_COLOR = "yellow"  (Mantine yellow.6 #fab005)
BG_TOP = (26, 27, 30)        # #1a1b1e — the docs site's dark surface
BG_BOTTOM = (17, 20, 26)     # a shade deeper, for a gradient with a direction
ACCENT = (250, 176, 5)       # #fab005 — Mantine yellow, this site's theme-color
TEXT = (245, 246, 247)
MUTED = (150, 158, 168)
...
def build_card(artwork: Path | None, brand: str, tagline: str, domain: str) -> Image.Image:
    card = vertical_gradient((WIDTH, HEIGHT), BG_TOP, BG_BOTTOM).convert("RGBA")
    draw = ImageDraw.Draw(card)

    # --- artwork, right ----------------------------------------------------
    # OPTIONAL HERE, unlike the template, which hard-requires assets/ddb.png.
    # This repo has no logo asset: the site's mark is a live Iconify glyph
    # (noto:grinning-face-with-smiling-eyes in components/header.py), not a
    # file. Rendering that glyph would mean depending on a colour-emoji font
    # and would produce a different card on macOS and on Linux — for an image
    # that must be byte-stable across regenerations, that is the wrong trade.
    #
    # With no artwork the text block simply uses the full width. Pass
    # `--artwork <file.png>` when there is a real mark to place.
    if artwork is not None:
        # `thumbnail` preserves aspect ratio, so a square-ish logo and a wide
        # one both land inside the same box without being stretched. The alpha
        # bbox is cropped first: a logo with transparent margin would otherwise
        # be centred as if the margin were part of the image.
