...
def create_appshell(data):
    return dmc.MantineProvider(
        ...
        theme={
            ...
            # Custom Colors
            "colors": {
                # emoji-mart's own accent yellow, extended into a Mantine
                # 10-step scale so PRIMARY_COLOR="yellow" reads as the picker's
                # palette rather than Mantine's default gold.
                "yellow": [
                    "#FFF9DB",
                    "#FFF3BF",
                    "#FFEC99",
        ...
        children=[
            # a11y (template 1.6.41): the first tab stop jumps past the
            # sidebar's stops straight to the page content; visible only on
            # keyboard focus — `.skip-link` in assets/main.css.
            ...
            dmc.AppShell(
                [
                    ...
                    dmc.AppShellMain(
                        ...
                        style={
                            "minHeight":
                                f"calc(100dvh - {HEADER_HEIGHT + FOOTER_HEIGHT}px)"
                        },
                    ),
                    create_footer(),
                ],
                ...
                # at the bottom of this file re-evaluates it per navigation
                # (lib/aside.py). Reserved on EVERY page is what left
                # /changelog sitting in the docs column with an empty right
                # gutter.
                aside=aside_config("/"),
                withBorder=True,
            ),
...
def _collapse_aside_without_toc(pathname):
    """Full width for pages that render no aside: /changelog, /api, home and
