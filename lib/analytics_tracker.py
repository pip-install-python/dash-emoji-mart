...
class AnalyticsTracker:
    ...
    def track_visit(self, path, user_agent, ip_address=None, headers=None):
        ...
        # `detect_device_type`, and not as a read-time filter in
        # lib/traffic_rollup.py, for two reasons:
        #
        #   1. classification would run first, and the hub's health sweep and
        #      the CI smoke batteries land on the classifier's CRAWLER lane
        #      (a library UA is a crawler to `classify()`, and so is an absent
        #      one) — they would go into `bot_hits` and be reported to the hub
        #      as crawler interest in these docs;
        ...
        # hits it far more often than that, so storing it would turn the ledger
        # into a record of monitoring rather than of readers.
        skip_paths = [
