...
This copy serves the dash-emoji-mart docs site at https://emojimart.2plot.dev.
...
# The directory key, not the PyPI package name. This defaulted to
# "dash-emoji-mart" while lib/satellite_reporter.py already said "emojimart",
# so the same app arrived at the hub under two ids and its /admin/ad-board rows
# sat apart from its traffic rows. STANDARD §5: one short id everywhere.
APP_ID = os.environ.get("AD_APP_ID", APP_KEY)

_TIMEOUT = 2          # seconds per fetch — never stall a page view longer
_COOLDOWN = 60        # seconds to skip fetches after a failure
_session = requests.Session()

# Outbound half of the internal-traffic contract. Without this the session
# sends `python-requests/x.y`, which 2plot.dev's own tracker classifies as a
# bot — so every page view here inflated the HUB's bot_hits by one. One header
# on the session covers every call it makes.
...
def fetch_ad(page: str) -> dict | None:
    ...
    try:
        resp = _session.get(
            ...
            # were being counted as crawler traffic on the hub. Set per call as
            # well as on the session, because that is the surface the network's
            # shared test asserts and a session default is easy to lose in a
            # refactor. See lib/constants.INTERNAL_UA.
