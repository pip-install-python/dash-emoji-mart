...
Copied from dash-documentation-boilerplate 1.2.4. Only the ``app_id()`` source
differs per repo; here it is ``lib.satellite_reporter.app_key`` (leaflet reads
``satellite_analytics`` instead — same idea, different module name).
...
THE DEPLOY-TIME TRAP (LESSONS §10)
----------------------------------
``NETWORK_BULLETIN_URL`` must be set on the Render SERVICE. Blueprint
``envVars`` in render.yaml apply on Blueprint sync, NOT on git-push
autodeploys, so a satellite whose render.yaml lists the variable can still boot
without it forever. The visible symptom is one generic tip in the viewer banner
where the hub publishes several — which reads as a styling difference, not as a
missing integration. The boot line this module feeds is the cheap check.
...
# render.yaml and the docs have one place to copy from.
...
def app_id() -> str:
    """This app's key in the hub's network directory — ``emojimart``.
    ...
    automatically identified the same way here. A satellite left announcing
    itself as "boilerplate" would receive the template's announcements, and
    the hub's "who is rendering the bulletin" view would count it as that repo.
    """
...
def configure() -> bool:
    ...
    outage must not take this documentation down with it.
    """
    endpoint = url()
    if not endpoint:
        return False
