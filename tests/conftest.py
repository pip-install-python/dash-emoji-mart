...
_TMP_STATE = tempfile.mkdtemp(prefix="emojimart-tests-")
...
# is the one boot line the gate wave accepts a host on.
...
# The /iconify page builds its categories AT IMPORT TIME from
# api.iconify.design. Left alone, importing run.py in a secretless CI container
# would make the whole suite depend on a third-party CDN being reachable — and
# the failure would present as "the app does not boot", not as "the network is
# down". The loader already degrades to an empty category list on any error, so
# pointing its cache at the throwaway state dir keeps the tests hermetic and
# leaves nothing behind in the repo.
os.environ.setdefault("DASH_EMOJI_MART_CACHE_DIR",
                      os.path.join(_TMP_STATE, "iconify-cache"))
...
def pages(app_module):
    ...
    prose-substance sweeps would flag exactly the behaviour we want.
