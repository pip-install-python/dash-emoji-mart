...
# ------------------------------------------------------------- the manifest --
#
# NOT PORTED, and the gap is deliberate rather than an oversight.
#
# The donor pins a PWA surface — `assets/favicon/site.webmanifest`, a 192/512
# icon pair, an apple-touch-icon, and a theme-color that has to agree with the
# manifest. This repo has no `assets/favicon/` at all: it serves Dash's default
# favicon and declares `theme-color` in templates/index.html with nothing to
# reconcile it against. Copying the donor's assertions would have produced
# seven tests failing against a surface that does not exist, which is noise,
# not coverage.
#
# What is missing, if someone builds it later:
#   * assets/favicon/site.webmanifest  (name=SITE_BRAND, short_name,
#     start_url="/", display="standalone", theme_color=#fab005)
#   * 192x192 and 512x512 PNG icons, plus apple-touch-icon.png
#   * <link rel="manifest"> and <link rel="apple-touch-icon"> in the template
#
# At that point, re-copy this block from dash-documentation-boilerplate 1.2.4
# rather than rewriting it — including
# `test_every_asset_the_template_references_resolves` below, which is already
# here and is the one that catches the real failure mode: a template that
# references an icon the repository never committed.


def test_the_theme_colour_is_declared(client):
    """No manifest to reconcile against, so this checks only that it exists.

    The donor asserts theme-color == manifest.theme_color. Here the constant
    that must agree with it is scripts/make_social_card.py's ACCENT, which is
    what colours the card's rule and domain strip.
    """
    declared = _meta(client.get("/").text, "theme-color")
    assert declared, "no theme-color"
    assert declared[0].lower() == "#fab005", (
        f"theme-color is {declared[0]!r}; the card's ACCENT is #fab005"
    )
