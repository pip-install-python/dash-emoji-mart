...
def _declared_lastmods() -> set[str]:
    ...
    for md in Path("docs").glob("**/*.md"):
        ...
        if m:
            dates.add(m.group(1))
    # Generated pages declare theirs from their SOURCE (template 1.6.41):
    # /changelog's is the newest dated release heading in CHANGELOG.md,
    # /api's the committed extract's `generated` stamp — both move exactly
    # when the content moves, which is what "declared" means here. This pin
    # fired correctly when /api gained its lastmod; a date with no declared
    # source is still the lie that gets the whole sitemap discarded.
    try:
