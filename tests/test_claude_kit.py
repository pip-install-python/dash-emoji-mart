...
def test_sync_specs_are_specifiable():
    ...
    This is a pin on the AUTHOR of sync specs. This repo CONSUMES them:
    it publishes no releases and ships no `sync/` directory, so the pin
    skips rather than being deleted — grow a sync/ here and it wakes up
    on its own. Recorded in DIVERGENCES.md.
    """
    import pytest

    sync_dir = REPO / "sync"
    if not sync_dir.is_dir():
        pytest.skip("this fork consumes sync specs, it does not author them")
