...
NEW HERE on 2026-08-26, and its absence is the whole story of that day.
SYNC-1.6.22-1.6.29 item 6 calls this file fork-owned and never cargo, and
records emojimart among the forks whose stubs "matched signatures" in the
1.6.28 round. This fork had no such file at all — so the byte-identical
smoke_live.py landed through the fan-out with nothing offline exercising it,
CI went green, and the FIRST thing to run it was CD against production, where
its new crawler/browser identity parity block failed eight checks (run
33002001965). The 1.6.29 retrospective reclassed the file to contract because
seven forks' legacy `fetch` stubs TypeErrored; a fork with NO stub fails the
other way, silently and one lane later. Reverting either half of the
templates/index.html fix that shipped with this file reproduces all eight of
those failures here, offline, in about two seconds.

Ported from the template's reference at 1.6.29 (5589318) rather than written
fresh: what makes it fork-specific is already indirected through
lib.constants — BASE_URL, the canonical host every stub derives, and the
og:image URL and dimensions — so nothing in the body needed a fork's name in
it. Keeping the body diffable against upstream is deliberate; the fork-owned
part is this header and whatever this host needs that the template does not.
"""
