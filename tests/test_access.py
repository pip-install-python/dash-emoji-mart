...
# This repo's own pages, not the template's /networks and /backends. Both must
# be real registered routes: the crawler-head assertions below read a canonical
# tag out of the rendered document, and a path that does not exist renders a
# 404 with no canonical at all — which fails as "the key leaked" when what
# actually happened is "the page was never there".
#
# GATED_PAGE is only ever gated by the tests themselves (this site ships every
# page public); it just needs to be a page that exists.
GATED_PAGE = "/theming"
PUBLIC_PAGE = "/popover"
